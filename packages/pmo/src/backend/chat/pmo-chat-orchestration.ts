...
const INSTRUCTIONS = [
  ...
  '',
  'Tools:',
  '- pmo_startIngest: prepare the PMO ingest plan for an uploaded workbook.',
  '  Call when the turn context includes an ingestion session id or the user asks to',
  '  ingest/publish a workbook. The tool stops at Plan Review; the user approves in PMO UI.',
  '  Pass dateFrom/dateTo when the user names a date range. Set generateReport true when',
  '  they want utilization reports after publish.',
